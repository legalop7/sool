...
TOKEN = '7048415955:AAHMZG-JMblz6s0jJ5wigwCfioGKHOl9h6I'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -1002420845644
CHANNEL_ID = -1002420845644
error_channel_id = -1002420845644
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @LegalMods.*", parse_mode='Markdown')  # Contact owner message
            return
...
def process_attack_command(message):
    try:
        ...
        if target_port in blocked_ports:
            ...
            return
        if duration >= 1201:
            bot.send_message(message.chat.id, "*⏳ Maximum duration is 1200 seconds.*\n"  # Duration limit message
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @LegalMods* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @LegalMods* 🙏"  # Contact owner message
    else:
